...
export default defineComponent({
  ...
  methods: {
    ...
    brick (count: number) {
      ...
      if (this.coins.length && this.coins[0].pos.x < -this.width) { this.coins.splice(0, 1); }
      if (this.structures[this.structures.length - 1].pos.x > this.width) { return; }
      let x = this.width;
    ...
    printObjects (items: string | any[]) {
      ...
      if (this.players[0].pos.x + this.players[0].w * speed < this.width / 2) { speed = 0; }
      for (let i = 0; i < items.length; i++) {
